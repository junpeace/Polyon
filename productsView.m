...
@synthesize dataArr, productCollectionView;
...
-(void) callAPI_getProductAndCategory
{
    ...
    getProductAndCategory *request = [[getProductAndCategory alloc] init_getProductAndCategory: @"0"];
...
-(void) callAPI_getProductCategory
{
    ...
    getProductCategory *request = [[getProductCategory alloc] init_getProductCategory: @"0"];
...
-(void) callAPI_getProducts
{
    ...
    getProducts *request = [[getProducts alloc] init_getProducts: @"0"];
...
-(void) setUpView
{
    ...
    [button addTarget:self action:@selector(viewCart) forControlEvents:UIControlEventTouchDown];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([pc.type isEqualToString: @"Category"])
    {
        [lblName setBackgroundColor: [UIColor colorWithRed: 249/255.0f green: 192/255.0f blue: 9/255.0f alpha:1]];
        [lblName setText: pc.categoryName];
        [imgProduct setImageWithURL: [NSURL URLWithString: pc.categoryImageURL] placeholderImage: [UIImage imageNamed: @"default_productscategory_1"]];
    }
    else
    {
        [lblName setBackgroundColor: [UIColor colorWithRed: 127/255.0f green: 214/255.0f blue: 254/255.0f alpha:1]];
        [lblName setText: pc.productName];
    ...
    if (responseMessage != nil)
    {
        if ([[responseMessage objectForKey:@"action"] isEqualToString:@"getProductCategory"])
        {
            ...
            for(int i = 0; i < [[responseMessage objectForKey: @"data"] count]; i++)
            {
                ...
                [pc setType: @"Category"];
                [pc setCategoryID: [[[responseMessage objectForKey: @"data"] objectAtIndex: i] objectForKey: @"category_id"]];
                [pc setCategoryName: [[[responseMessage objectForKey: @"data"] objectAtIndex: i]objectForKey: @"category_name"]];
                [pc setCategorySequenceNo: [[[responseMessage objectForKey: @"data"] objectAtIndex: i] objectForKey: @"sequence_no"]];
                [pc setCategoryImageURL: [[[responseMessage objectForKey: @"data"] objectAtIndex: i] objectForKey: @"category_image_url"]];
        ...
        else if ([[responseMessage objectForKey:@"action"] isEqualToString:@"getProductsCategoriesByCategoryId"])
        {
            ...
            for(int i = 0; i < [catArr count]; i++)
            {
                productCategory *pc = [[productCategory alloc] init];
                [pc setType: @"Category"];
                [pc setCategoryID: [[catArr objectAtIndex: i] objectForKey: @"category_id"]];
                [pc setCategoryName: [[catArr objectAtIndex: i] objectForKey: @"category_name"]];
                [pc setCategorySequenceNo: [[catArr objectAtIndex: i] objectForKey: @"sequence_no"]];
                [pc setCategoryImageURL: [[catArr objectAtIndex: i] objectForKey: @"category_image_url"]];
